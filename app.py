...
def render_periodic_trend(data, group_col, label):
    ...
    c1, c2 = st.columns(2)
    with c1:
        st.write(f"🏢 **{label} 거래처별 (RN, 매출, ADR)**")
    ...
    with c2:
        st.write(f"🛏️ **{label} 객실 타입별 (RN, 매출, ADR)**")
    ...
    if len(raw) > 1:
        ...
        with tab_sum:
            ...
            if len(all_dates) >= 2:
                ...
                m3.metric("최근 취소 발생", f"{len(db_df[(db_df['Snapshot_Date']==latest) & (db_df['Status']=='Cancelled')])}건")
                m4.metric("픽업 ADR", f"{pick_rev/pick_rn if pick_rn!=0 else 0:,.0f}원")
            st.divider()
            c1, c2 = st.columns([2, 1])
            with c1:
                st.plotly_chart(px.bar(bk.groupby('Stay_Month')['Room_Revenue'].sum().reset_index(), x='Stay_Month', y='Room_Revenue', title="투숙월별 점유 매출"), use_container_width=True)
            with c2:
                st.plotly_chart(px.pie(bk, values='Room_Revenue', names='Segment', hole=0.4, title="채널 비중"), use_container_width=True)
