...
def process_data(uploaded_file, status, sub_segment="General"):
    try:
        ...
        if is_otb:
            ...
            df['Segment'] = f'OTB_{sub_segment}' # 중요: 이것으로 OTB 데이터 식별
...
# 상세 분석 렌더링 함수 (지배인님이 좋아하셨던 기능 복구)
def render_full_analysis(data, title):
    ...
    with c1:
        ...
        st.dataframe(acc.sort_values('Room_Revenue', ascending=False), hide_index=True, use_container_width=True)
    with c2:
        ...
        st.dataframe(rt.sort_values('Room_Revenue', ascending=False), hide_index=True, use_container_width=True)

    # 2. 리드타임 & 국적 (복구 완료)
    c3, c4 = st.columns(2)
    with c3:
        st.caption("⏳ 리드타임 분석 (예약 시점)")
        # 리드타임 구간화
        bins = [-999, 0, 3, 7, 14, 30, 60, 999]
        labels = ['Dang-il', '1-3 Days', '4-7 Days', '8-14 Days', '15-30 Days', '31-60 Days', '60+ Days']
...
try:
    ...
    with st.sidebar.expander("🎯 세일즈 온더북 (버짓용)", expanded=True):
        ...
        if f3 and st.button("당월 OTB 반영"):
            df = process_data(f3, "Booked", "Month") # Segment: OTB_Month
            if not df.empty:
                db_sheet.append_rows(df.fillna('').astype(str).values.tolist())
                load_data_from_sheet.clear()
                st.success("반영 완료!")
        ...
        if f4 and st.button("전체 OTB 반영"):
            df = process_data(f4, "Booked", "Total") # Segment: OTB_Total
            if not df.empty:
                db_sheet.append_rows(df.fillna('').astype(str).values.tolist())
                load_data_from_sheet.clear()
                st.success("반영 완료!")
    ...
    if len(raw_data) <= 1:
        ...
        # 데이터 분리 (지배인님 요청사항: 영역 분리)
        # 1. OTB 데이터 (버짓용)
        df_otb_m = df[df['Segment'] == 'OTB_Month']
        df_otb_t = df[df['Segment'] == 'OTB_Total']
        
        # 2. 리스트 데이터 (인사이트용) - OTB가 아닌 것들
        df_list = df[~df['Segment'].str.contains('OTB')]
        df_list_bk = df_list[df_list['Status'] == 'Booked'] # 순수 예약
        df_list_cn = df_list[df_list['Status'] == 'Cancelled'] # 순수 취소

        # ======================================================================
        # [영역 1] 세일즈 온더북 버짓 달성현황 (OTB 파일 기반)
        # ======================================================================
        st.markdown("### 🎯 세일즈 온더북 버짓 달성현황 (Source: OTB File)")
        st.caption("※ 이 영역은 '세일즈 온더북' 파일 데이터로만 계산됩니다.")
        ...
        st.caption("※ 이 영역은 '신규 예약 리스트'와 '취소 리스트' 데이터로만 분석됩니다.")
        ...
        if df_list.empty:
            ...
            with tab1:
                ...
                with col_a:
                    st.write("**월별 예약 유입 (Booking Date 기준)**")
                    monthly_bk = df_list_bk.groupby('Stay_Month')['Room_Revenue'].sum().reset_index()
                    st.bar_chart(monthly_bk.set_index('Stay_Month'))
                with col_b:
                    st.write("**주별 순매출 추이 (예약-취소)**")
                    # 취소는 마이너스 처리
                    cn_neg = df_list_cn.assign(Room_Revenue = -df_list_cn['Room_Revenue'])
                    net_df = pd.concat([df_list_bk, cn_neg])
                    weekly = net_df.groupby('Stay_YearWeek')['Room_Revenue'].sum().reset_index()
            ...
            with tab4:
                ...
                st.write(f"총 {len(zero)}건의 0원/무료 예약이 있습니다.")
                st.dataframe(zero[['Guest_Name', 'CheckIn', 'Account', 'Room_Type', 'Remark'] if 'Remark' in zero.columns else ['Guest_Name', 'CheckIn', 'Account', 'Room_Type']])

            with tab5:
                # 합계 데이터 (리스트 합산)
                st.metric("리스트 합산 총 매출", f"{df_list_bk['Room_Revenue'].sum():,.0f}원")
                st.metric("리스트 합산 총 룸나잇", f"{df_list_bk['RN'].sum():,.0f} RN")
