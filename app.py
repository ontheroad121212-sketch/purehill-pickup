...
# 0. 스타일 설정
# ------------------------------------------------------------------------------
st.set_page_config(page_title="ARI Segment Master", layout="wide")
st.markdown("""
<style>
...
def normalize_and_map_columns(df):
    ...
    rules = {
        ...
        # [핵심 수정] Market, Mkt 제외 -> 오직 'Segment', '세그먼트'만 인정
...
def process_data(uploaded_file, status, sub_segment="General"):
    try:
        ...
        if is_otb:
            ...
            for c in req_cols:
                if c not in df.columns: 
                    ...
                    else: df[c] = 'Unknown' # 세그먼트 없으면 Unknown 처리
        ...
        # 리드타임: 원본 O열(Lead_Time)이 있으면 쓰고, 없거나 0이면 계산
        if is_otb:
             df['Lead_Time'] = 0
        else:
            # Lead_Time 컬럼이 파일에 존재하면 일단 가져오기 시도
            if 'Lead_Time' in df.columns:
                df['Lead_Time'] = pd.to_numeric(df['Lead_Time'], errors='coerce').fillna(0)
            else:
                df['Lead_Time'] = 0
            
            # 0이 너무 많으면(대부분이면) 계산 로직으로 덮어씀 (지배인님 요청)
            if df['Lead_Time'].sum() == 0:
...
try:
    ...
    st.title("🏛️ 앰버 호텔 경영 리포트 (Segment Master)")
    ...
    if len(raw_data) <= 1:
        ...
        if df_list.empty:
            ...
            # --- Tab 1: Segment Analysis (NEW) ---
            with tab1:
                st.subheader("📊 세그먼트(Segment)별 실적 분석")
                
                # 유료 예약 기준 세그먼트 통계
                seg_stats = df_paid_bk.groupby('Segment').agg({
                    'RN': 'sum', 
                    'Room_Revenue': 'sum', 
                    'Total_Revenue': 'sum'
                }).reset_index()
                ...
                # Insight
                top_seg = seg_stats.sort_values('Room_Revenue', ascending=False).head(1)
                if not top_seg.empty:
                    seg_name = top_seg['Segment'].values[0]
                    seg_share = (top_seg['Room_Revenue'].values[0] / seg_stats['Room_Revenue'].sum()) * 100
                    st.info(f"💡 Insight: '{seg_name}' 세그먼트가 전체 객실 매출의 {seg_share:.1f}%를 견인하고 있습니다.")

                col1, col2 = st.columns(2)
                col1.plotly_chart(px.pie(seg_stats, values='Room_Revenue', names='Segment', title="세그먼트별 매출 비중"), use_container_width=True)
                col2.plotly_chart(px.bar(seg_stats, x='Segment', y='ADR', title="세그먼트별 ADR 비교", text_auto=',.0f', color='Segment'), use_container_width=True)
            ...
            with tab2:
                st.subheader("🗓️ 예약 시점별 입실 분포 (Pacing Matrix)")
                pivot_metric = st.radio("기준", ["객실수 (RN)", "객실매출"], horizontal=True)
                ...
                fig = px.imshow(pacing, text_auto=True if "RN" in pivot_metric else ".2s", aspect="auto", color_continuous_scale="Blues")
                st.plotly_chart(fig, use_container_width=True)

            with tab3:
                st.subheader("🏢 거래처 분석")
                acc_stats = df_paid_bk.groupby('Account').agg({'RN':'sum', 'Room_Revenue':'sum', 'Total_Revenue':'sum'}).reset_index()
                acc_stats['ADR'] = (acc_stats['Room_Revenue'] / acc_stats['RN']).fillna(0)
                fig_acc = px.scatter(acc_stats, x="RN", y="ADR", size="Room_Revenue", color="Account", hover_name="Account", log_x=True, size_max=60)
                st.plotly_chart(fig_acc, use_container_width=True)
                st.dataframe(acc_stats.sort_values('Room_Revenue', ascending=False), column_config={"Room_Revenue": st.column_config.NumberColumn(format="%d원"), "ADR": st.column_config.NumberColumn(format="%d원")}, hide_index=True, use_container_width=True)
            ...
            with tab4:
                ...
                fig_lead.update_layout(yaxis2=dict(overlaying='y', side='right', title='ADR'))
                st.plotly_chart(fig_lead, use_container_width=True)

            with tab5:
                st.subheader("🛏️ 객실타입 효율")
                rt_stats = df_paid_bk.groupby('Room_Type').agg({'RN':'sum', 'Room_Revenue':'sum', 'Total_Revenue':'sum'}).reset_index()
                rt_stats['ADR'] = (rt_stats['Room_Revenue'] / rt_stats['RN']).fillna(0)
                st.dataframe(rt_stats.sort_values('Room_Revenue', ascending=False), column_config={"Room_Revenue": st.column_config.NumberColumn(format="%d원"), "ADR": st.column_config.NumberColumn(format="%d원")}, hide_index=True, use_container_width=True)

            with tab6:
                st.subheader("🗓️ 요일별 패턴")
                wd_stats = df_paid_bk.groupby('Day_Type').agg({'RN':'sum', 'Room_Revenue':'sum'}).reset_index()
                wd_stats['ADR'] = (wd_stats['Room_Revenue'] / wd_stats['RN']).fillna(0)
                c1, c2 = st.columns(2)
                c1.plotly_chart(px.bar(wd_stats, x='Day_Type', y='ADR', title="주중/주말 ADR", text_auto=',.0f'), use_container_width=True)
                c2.plotly_chart(px.pie(wd_stats, values='RN', names='Day_Type', title="점유율 비중"), use_container_width=True)

            with tab7:
                st.subheader("❌ 취소 분석")
                if not df_list_cn.empty:
                    cn_stats = df_list_cn.groupby('Account').agg({'RN':'sum', 'Room_Revenue':'sum'}).reset_index()
                    st.dataframe(cn_stats.sort_values('RN', ascending=False), column_config={"Room_Revenue": st.column_config.NumberColumn(format="%d원")}, hide_index=True, use_container_width=True)
                else:
