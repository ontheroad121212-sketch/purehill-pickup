...
def process_data(uploaded_file, status):
    ...
    df['Stay_Month'] = df['CheckIn'].apply(lambda x: x[:7])
    ...
    final_cols = ['Guest_Name', 'CheckIn', 'Booking_Date', 'RN', 'Room_Revenue', 'Total_Revenue', 'ADR', 'Segment', 'Account', 'Room_Type', 'Snapshot_Date', 'Nat_Group', 'Status', 'Stay_Month', 'Lead_Time', 'Day_of_Week', 'Month_Label']
    return df[final_cols], today_str

# 3. 무삭제 분석 렌더링 (모든 탭에서 동일하게 호출)
...
# --- UI 메인 ---
st.set_page_config(page_title="ARI Extreme Pro", layout="wide")
st.title("🏨 Amber Revenue Intelligence (ARI)")

tab_up, tab_sum, tab_det = st.tabs(["📤 데이터 업로드", "📋 경영진 요약", "📈 무삭제 상세 분석"])
    ...
    if len(raw) > 1:
        ...
        with tab_sum:
            ...
            if len(dates) >= 2:
                ...
                c3.metric("최근 취소 건수", f"{len(db_df[(db_df['Snapshot_Date']==latest) & (db_df['Status']=='Cancelled')])} 건", delta_color="inverse")
            st.divider()
            # 총 예약 현황
            st.subheader("💎 보유 예약 총계 (Gross Booking)")
            k1, k2, k3, k4 = st.columns(4)
            k1.metric("총 보유 RN", f"{bk['RN'].sum():,.0f}")
            k2.metric("총 보유 매출", f"{bk['Room_Revenue'].sum():,.0f}")
            k3.metric("평균 리드타임", f"{bk['Lead_Time'].mean():,.1f} 일")
            k4.metric("실질 넷 픽업(Net)", f"{net_df['RN'].sum():,.0f}")
