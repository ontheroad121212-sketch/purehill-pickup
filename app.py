...
try:
    ...
    if len(raw) > 1:
        ...
        cn = paid_df[paid_df['Status'] == 'Cancelled']

        # --- [최상단] 유료 실적 8대 KPI 대시보드 ---
        st.header(f"🏛️ 앰버 호텔 경영 요약 리포트 ({sel_date})")
        st.info("💡 총금액이 0원인 예약(체험단, VIP 등)은 모든 매출 지표에서 제외되었습니다.")
        ...
        k5.metric("❌ 유료 취소 RN", f"{c_rn:,.0f} RN", delta_color="inverse")
        k6.metric("📉 유료 취소 총 매출", f"{c_rev:,.0f} 원", delta_color="inverse")
        k7.metric("🔻 유료 취소 객실 매출", f"{c_room:,.0f} 원", delta_color="inverse")
        k8.metric("📊 취소 ADR", f"{(c_room/c_rn if c_rn>0 else 0):,.0f} 원", delta_color="inverse")
        
        st.divider()

        # 월별 유료 실적 및 버짓 달성률
        st.subheader("📅 월별 유료 실적 및 목표 달성률 (Performance & Budget)")
        ...
        with tab_zero:
            ...
            if not zero_booked.empty:
                st.dataframe(zero_booked[['Guest_Name', 'CheckIn', 'RN', 'Account', 'Segment', 'Room_Type']], use_container_width=True)
                st.caption(f"총 {len(zero_booked)}건의 유효한 0원 예약이 조회되었습니다.")
            else:
                st.write("조회된 0원 예약 내역이 없습니다.")
