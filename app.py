...
# 2. 데이터 처리 엔진 (지배인님 원본 로직 100% 유지 + 영업현황 분리 대응)
...
try:
    ...
    if len(raw_db) > 1:
        ...
        # 1. 당월 실적 (Segment == 'OTB_Month' 에서 추출)
        ...
        # 2. 전체 실적 (Segment == 'OTB_Total' 에서 추출)
        ...
        with colA:
            ...
            c3.metric("ADR", f"{m_adr:,.0f} 원"); c4.metric("룸나잇", f"{m_rn:,.0f} RN")
        with colB:
            st.info("🌍 전체 기간 누적 버짓 현황")
            k1, k2, k3, k4 = st.columns(4)
            k1.metric("전체 달성률", f"{t_achieve:.1f} %", delta=f"T: {t_budget:,.0f}")
            k2.metric("누적 매출", f"{t_rev:,.0f} 원")
            k3.metric("누적 ADR", f"{t_adr:,.0f} 원"); k4.metric("누적 RN", f"{t_rn:,.0f} RN")
        ...
        with tab_det:
            res_bk = paid_df[(~paid_df['Segment'].str.contains('OTB')) & (paid_df['Status'] == 'Booked')]
            res_cn = f_df[(~f_df['Segment'].str.contains('OTB')) & (f_df['Status'] == 'Cancelled')]
        ...
        with tab_up:
            st.subheader("📤 데이터 통합 업데이트 (3개 파일 섹션)")
            ...
            with c1:
                ...
                if f1:
                    ...
                    if st.button("반영하기"):
                        ...
                        st.success("완료!")
            with c2:
                f_otb_m = st.file_uploader("2️⃣ 세일즈온더북 (당월 전용)", type=['xlsx', 'csv'], key="f_otb_m")
                if f_otb_m and st.button("당월 OTB 반영"):
                    db_sheet.append_rows(process_data(f_otb_m, "Booked", "Month").fillna('').astype(str).values.tolist())
                    st.success("당월 반영 완료!")
            with c3:
                f_otb_t = st.file_uploader("3️⃣ 세일즈온더북 (전체 누적)", type=['xlsx', 'csv'], key="f_otb_t")
                if f_otb_t and st.button("전체 OTB 반영"):
